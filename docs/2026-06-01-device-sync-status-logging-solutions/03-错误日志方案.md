...
> **传输通道**：错误日志仅经 **WebSocket 长连接** 推送（与设备状态共用连接、分消息类型）；**不采用** 独立 `POST /api/error-logs` 轮询上传或定时 HTTP 批量上传。
...
{
    private readonly IDeviceStatusWebSocketService _webSocket;
    ...
    public async Task TransmitAsync(ErrorLogEntry logEntry)
    {
        try
        {
            await _webSocket.SendMessageAsync("error_log", logEntry);
            _logger.LogDebug("Error log sent via WebSocket: {LogId}", logEntry.Id);
        }
        catch (Exception ex)
        {
    ...
    public async Task TransmitCriticalAsync(ErrorLogEntry logEntry)
    {
        try
        {
            await _webSocket.SendMessageAsync("error_log_critical", logEntry);
        }
        catch (Exception ex)
        {
...
**4. Hub 消息处理 + 管理端查询 API**

```csharp
// src/UrbanManagement.App/Hubs/DeviceStatusHub.cs（节选）
public async Task OnMessageAsync(string messageType, JsonElement payload)
{
    switch (messageType)
    {
        case "error_log":
            await _ingestionService.IngestAsync(payload.Deserialize<ErrorLogEntry>()!);
            break;
        case "error_log_critical":
            await _ingestionService.IngestCriticalAsync(payload.Deserialize<ErrorLogEntry>()!);
            break;
    }
}
```
...
连接断开时，失败日志写入本地队列，**重连后**经同一 WebSocket 按条或聚合帧补传（消息类型 `error_log_replay`），不使用独立 HTTP 批量接口。
