...
称重记录仍沿用现有 **Refit HTTP**（`POST /api/urban/weighing-records`），与长连接通道职责分离。
...
| 01 | [01-设备实时同步方案.md](./01-设备实时同步方案.md) | MaterialClient.Urban 到 UrbanManagement 的设备同步（WebSocket） |
...
### 技术债务
- 需要建立 **WebSocket** 实时通信机制（单连接多消息类型）
- 需要设计设备状态数据模型
