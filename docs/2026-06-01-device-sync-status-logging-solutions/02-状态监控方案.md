...
> **上报通道**：心跳、客户端状态、摄像头状态均经 **WebSocket 长连接** 推送（`IDeviceStatusReporter` / `DeviceStatusHub`）；不使用 HTTP 定时轮询上报。本地摄像头探测可为定时任务，与上行通道无关。
...
public class CameraStatusService : ICameraStatusService
{
    ...
    public async Task<Dictionary<string, CameraStatusInfo>> GetAllCameraStatusAsync()
    {
        ...
        // 经 WebSocket 聚合上报（消息类型如 camera_status）
